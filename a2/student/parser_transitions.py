...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            head, deps = self.stack[-1], self.stack[-2]
            self.dependencies.append((head, deps))
            self.stack.pop(-2)
        elif transition == "RA":
            head, deps = self.stack[-2], self.stack[-1]
            self.dependencies.append((head, deps))
            self.stack.pop(-1)
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies: list = [None] * len(sentences)
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    idx_of = {id(pp): i for i, pp in enumerate(partial_parses)}
    unfinished_parses = partial_parses.copy()
    while unfinished_parses:
        minibatch = unfinished_parses[:batch_size]
        predict_acts = model.predict(minibatch)
        for i, sentence in enumerate(minibatch):
            sentence.parse_step(predict_acts[i])
            if sentence.buffer == [] and len(sentence.stack) == 1:
                origin_idx = partial_parses.index(sentence)
                dependencies[idx_of[id(sentence)]] = sentence.dependencies
