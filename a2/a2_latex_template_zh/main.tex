...
\textbf{截止日期：4 月 18 日（周四），美国太平洋时间下午 4:30。}

在这个作业中，你将复习 Word2Vec 背后的数学原理，并使用 PyTorch 构建一个神经依存句法分析器。关于 PyTorch 基础知识的复习，请查看 Canvas 上的 PyTorch 复习课。第 1 部分中，你将探索使用 naive softmax loss 训练 Word2Vec 模型时涉及的偏导数。第 2 部分中，你将学习两种通用神经网络技巧（Adam 优化和 Dropout）。第 3 部分中，你将实现并训练一个依存句法分析器，并使用第 2 部分中的技术，在分析几个错误依存句法解析的同时完成训练。

如果你使用 LaTeX，可以使用 \verb!\ifans{}! 来输入你的解答。

\textbf{请在 Gradescope 上正确标记题目，否则如果你没有正确标记，助教会扣分。}
\begin{questions}
    \input{q1.tex}
    \newpage
    \input{q2.tex}
